 ...
 * 动态素材加载系统
 */
window.DynamicAssetLoader = {
  // 缓存的资源名称
  _resourceName: null,

  // 获取 URL 参数中的 assets 值
  getResourceFromURL: function() {
    if (this._resourceName === null) {
      const urlParams = new URLSearchParams(window.location.search);
      this._resourceName = urlParams.get('assets') || 'merge-melon'; // 默认值
      console.log('Dynamic Asset Loader: Using asset pack:', this._resourceName);
    }
    return this._resourceName;
  },

  // 构建素材路径
  buildAssetPath: function(filename) {
    const resourceName = this.getResourceFromURL();
    return `assets/${resourceName}/${filename}`;
  },

  // 构建回退路径（使用默认素材包）
  buildFallbackPath: function(filename) {
    return `assets/merge-melon/${filename}`;
  },

  // 检查素材是否存在（可选的错误处理）
  checkAssetExists: function(path, callback) {
    const img = new Image();
  ...
  // 获取素材路径，带回退机制
  getAssetPathWithFallback: function(filename, callback) {
    const primaryPath = this.buildAssetPath(filename);
    const fallbackPath = this.buildFallbackPath(filename);

    this.checkAssetExists(primaryPath, function(exists) {
      if (exists) {
        callback(primaryPath);
      } else {
        console.warn('Asset not found:', primaryPath, 'falling back to:', fallbackPath);
...
window.__require = function e(t, n, o) {
    ...
    var a = cc._decorator,
      ...
      s = function (e) {
        ...
        return o(t, e), n = t, t.prototype.onLoad = function () {
          ...
          // 检查是否有对应的线上音效
          // 通过全局查找 DynamicLoad 实例
          const dynamicLoadInstance = cc.find("Canvas").getComponent("DynamicLoad");
          if (dynamicLoadInstance && dynamicLoadInstance.onlineAudios && dynamicLoadInstance.onlineAudios[e]) {
            const audio = dynamicLoadInstance.onlineAudios[e];
            audio.currentTime = 0; // 重置播放位置
            audio.volume = n || 0.5;
            audio.play().catch(function(error) {
              console.error('Failed to play online audio:', e, error);
              // 如果线上音效播放失败，使用默认音效
    ...
    var a = cc._decorator,
      ...
      s = function (e) {
        ...
        return o(t, e), n = t, t.prototype.onLoad = function () {
          ...
          // 加载线上水果图片，替换原有的 SpriteFrame
          this.loadOnlineFruits();
          // 加载线上背景图片
          this.loadOnlineBackground();
          // 加载线上音效
          this.loadOnlineAudios();
        }, t.prototype.loadOnlineFruits = function() {
          // 动态构建水果图片URL数组
          const onlineFruitUrls = [
            window.DynamicAssetLoader.buildAssetPath('1.png'), // 0 - 葡萄
            window.DynamicAssetLoader.buildAssetPath('2.png'), // 1 - 樱桃
            window.DynamicAssetLoader.buildAssetPath('3.png'), // 2 - 草莓
            window.DynamicAssetLoader.buildAssetPath('4.png'), // 3 - 橘子
            window.DynamicAssetLoader.buildAssetPath('5.png'), // 4 - 柠檬
            window.DynamicAssetLoader.buildAssetPath('6.png'), // 5 - 猕猴桃
            window.DynamicAssetLoader.buildAssetPath('7.png'), // 6 - 菠萝
            window.DynamicAssetLoader.buildAssetPath('8.png'), // 7 - 椰子
            window.DynamicAssetLoader.buildAssetPath('9.png'), // 8 - 西瓜
            window.DynamicAssetLoader.buildAssetPath('10.png'), // 9 - 半个西瓜
            window.DynamicAssetLoader.buildAssetPath('11.png')  // 10 - 大西瓜
          ];

          // 记录加载完成的数量
          this.loadedCount = 0;
          this.totalCount = onlineFruitUrls.length;

          // 为每个水果位置加载对应的图片
          for (let i = 0; i < onlineFruitUrls.length; i++) {
            this.loadRemoteImage(onlineFruitUrls[i], i);
          }
        }, t.prototype.loadRemoteImage = function(url, index) {
          const self = this;

          cc.loader.load(url, function(err, texture) {
            if (err) {
              console.error('Failed to load fruit image for index:', index, 'URL:', url, 'Error:', err);
              self.loadedCount++;
              return;
            }
            ...
            // 只替换主要的 fruit 数组，保留原有的果汁特效素材
            if (self.fruit && self.fruit.length > index) {
              self.fruit[index] = spriteFrame;
            }
            // 注意：不替换 fruitL, guozhiZ, guozhiL，让合成特效使用原本的素材
            ...
            if (self.loadedCount >= self.totalCount) {
              self.allFruitsLoaded = true; // 设置标志，表示所有水果图片已加载完成
            }
          });
        }, t.prototype.loadOnlineBackground = function() {
          // 动态构建背景图片URL
          const backgroundUrl = window.DynamicAssetLoader.buildAssetPath('background_game.png');

          const self = this;

          cc.loader.load(backgroundUrl, function(err, texture) {
            if (err) {
              console.error('Failed to load background image:', backgroundUrl, 'Error:', err);
              return;
            }
            ...
            // 查找背景节点并设置背景图片
            const bgLayerNode = cc.find("Canvas/bgLayer");
            if (bgLayerNode) {
              // 检查是否已有 Sprite 组件，如果没有则添加
              let spriteComponent = bgLayerNode.getComponent(cc.Sprite);
              if (!spriteComponent) {
                spriteComponent = bgLayerNode.addComponent(cc.Sprite);
              }

              // 设置背景图片
              spriteComponent.spriteFrame = spriteFrame;
            } else {
              console.error('Background layer node not found');
            }
          });
        }, t.prototype.loadOnlineAudios = function() {
          // 动态构建音效文件映射
          const audioMapping = {
            3: window.DynamicAssetLoader.buildAssetPath('merge.mp3'),  // 合成音效1
            5: window.DynamicAssetLoader.buildAssetPath('down.mp3'),   // 下落音效
            6: window.DynamicAssetLoader.buildAssetPath('merge.mp3')   // 合成音效2（使用同一个文件）
          };

          const self = this;

          // 为每个需要替换的音效创建在线音频对象
          Object.keys(audioMapping).forEach(function(index) {
            const audioUrl = audioMapping[index];
            ...
            // 存储到自定义音频映射中
            if (!self.onlineAudios) {
              self.onlineAudios = {};
            }
            self.onlineAudios[audioIndex] = audio;
            ...
            audio.addEventListener('error', function(error) {
              console.error('Failed to load audio for index:', audioIndex, 'URL:', audioUrl, 'Error:', error);
            });
          });
  ...
  EffectCenter: [function (e, t, n) {
    ...
    var a = e("../Common/Utils"),
      ...
      f = function (e) {
        function t() {
          var t = null !== e && e.apply(this, arguments) || this;
          return t.bgEffect = null, t.EffectPartiParent = null, t.upEffectPartiParent = null, t.uiEffectParent = null, t.cameraNode = null, t.fireLightNode = null, t.effectTxtParent = null, t.bgEffectParent = null, t
        }
      ...
      m = function (e) {
        ...
        return o(t, e), n = t, t.prototype.onLoad = function () {
          ...
          a.default.playNum, a.default.bgNumber >= 3 && (a.default.bgNumber = 0), a.default.bgNumber++, a.default.playNum++, cc.tween(this.lineNode.children[0]).to(.3, {
            ...
            this.scheduleCreateFirstFruit(firstFruit) // 延迟创建第一个水果，等待图片加载
        }, t.prototype.scheduleCreateFirstFruit = function (fruitIndex) {
          var self = this;

          // 检查图片是否已加载
          function checkAndCreateFruit() {
            // 检查 DynamicLoad 实例和对应的水果图片
            if (d.default.Instance &&
                d.default.Instance.fruit &&
                d.default.Instance.fruit[fruitIndex] &&
                d.default.Instance.allFruitsLoaded) {
              // 图片已加载，创建水果
              console.log('First fruit images loaded, creating first fruit:', fruitIndex);
              self.createOneFruit(fruitIndex);
            } else {
              // 图片还没加载完成，继续等待
              console.log('Waiting for first fruit image to load, index:', fruitIndex,
                         'hasInstance:', !!d.default.Instance,
                         'hasFruit:', !!(d.default.Instance && d.default.Instance.fruit),
                         'hasSpecificFruit:', !!(d.default.Instance && d.default.Instance.fruit && d.default.Instance.fruit[fruitIndex]),
                         'allLoaded:', !!(d.default.Instance && d.default.Instance.allFruitsLoaded));
              self.scheduleOnce(checkAndCreateFruit, 0.2);
            }
          }
          ...
          t.prototype.createOneFruit = function (e) {
            ...
            cc.tween(n).to(.5, {scale: 1}, {
              ...
              // 获取标签页ID（iframe和父页面共享sessionStorage）
              const senderTabId = sessionStorage ? sessionStorage.getItem('gameTabId') : null;
              
              const messageData = {
                type: 'fruitGenerate',
                senderTabId: senderTabId,
                data: {
                  fruitName: fruitNames[e] || '未知水果',
                  fruitIndex: e,
                  position: { x: n.position.x, y: n.position.y },
                  timestamp: new Date().toLocaleTimeString(),
                  source: 'generate' // 标识为自动生成
                }
              };
              
              // 优先使用 PostMessage（iframe通信，天然隔离多标签页）
              if (window.parent && window.parent !== window) {
                try {
                  window.parent.postMessage(messageData, window.location.origin);
                } catch (error) {
                  console.warn('PostMessage 发送失败，尝试 BroadcastChannel:', error);
                  // 备用方案：使用 BroadcastChannel
                  if (typeof BroadcastChannel !== 'undefined') {
                    try {
                      const channel = new BroadcastChannel('fruit-merge-channel');
                      channel.postMessage(messageData);
                      channel.close();
                    } catch (bcError) {
                      console.error('BroadcastChannel 也发送失败:', bcError);
                    }
                  }
                }
              } else {
  ...
  HttpManagerJs: [function (e, t, n) {
    ...
    var a = e("GameConfig"),
      i = (c(o = {
        URL: "http://www.xxx.com/h5service.php/Interface/services",
        cacheList: null,
        isBusy: null,
        req: null,
        perform: null,
  ...
  LoadSceneJs: [function (e, t, n) {
    ...
    var o = {
      goToCover: function (e, t, n, o, c) {
        var a = e;
        a = null == a || void 0 == a || e, this.needShow = !1, a && a ? (this.needShow = !0, showMyAds()) : this.needShow = !1, this.needShow ? (void 0 == preloader && this.startGoToGame(n, o, c), resCompleteFlag = !0, adCompleteFlag && resCompleteFlag && this.startGoToGame(n, o, c)) : this.startGoToGame(n, o, c)
      },
    ...
    cc.Class({
      ...
      addTouchEvents: function () {
        ...
        this.node.on(cc.Node.EventType.TOUCH_START, function (t) {
          if (o.GAME_OVER_BOOL && o.noTouchBool) return o.gameScore++, e.sceneScore.string = o.gameScore, o.gameScore >= 2 && (this.lastPos = t.getLocation(), e.gameEnd()), !0
      ...
      m = function (e) {
        ...
        return o(t, e), t.prototype.onLoad = function () {
          ...
          if ("downwall" == n.node.group && (t.node.parent = cc.find("Canvas/fruitNode"), 0 == this.wallColl && (d.default.Instance.Play(5, !1, 1), this.wallColl++)), "fruit" == n.node.group) {
            ...
            // 水果名称映射
            var fruitNames = ["葡萄", "樱桃", "草莓", "橘子", "柠檬", "猕猴桃", "菠萝", "椰子", "西瓜", "半个西瓜"];
            // 控制合成水果的边界值
            ...
            c == r && (reverseLevelUp ? (c > border && r > border) : (c < border && r < border)) ? (this.pengzhuangCount += 1,
              0 == t.node.getComponent("fruitData").getNumber() && (a.default.score += this.fruitNumber + (extraScore ? extraScore : 1), // 改分数
                ...
                cc.tween(t.node).to(.1, {
                  ...
                  // 打印水果合成信息
                  var mergedFruitNumber = o.fruitNumber + fruitVolume;
                  var mergedFruitName = fruitNames[mergedFruitNumber];
                  ...
                  const messageData = {
                    ...
                    data: {
                      mergeType: 'normal',
                      fruit1: fruitNames[o.fruitNumber],
                      fruit2: fruitNames[o.fruitNumber],
                      result: mergedFruitName,
                      position: { x: n.node.position.x, y: n.node.position.y },
                      timestamp: new Date().toLocaleTimeString(),
                      source: 'merge' // 标识为用户合成
                    }
                  };
                  ...
                  i.default.Instance.createFruitSui(o.fruitNumber, n.node.position), i.default.Instance.createFruitL(o.fruitNumber, n.node.position, n.node.width),
                    ...
                    o.playCustomSound(o.fruitNumber + fruitVolume),
              ...
              0 == t.node.getComponent("fruitData").getNumber() && (a.default.score += this.fruitNumber + (extraScore ? extraScore : 1), // 改分数
                ...
                var s = new cc.Node;
                s.addComponent(cc.Sprite).spriteFrame = l.default.Instance.fruit[reverseLevelUp ? 0 : 10], s.parent = c, s.position = cc.v2(0), o.playCustomSound(o.fruitNumber + fruitVolume), i.default.Instance.ribbonEffect(cc.v2(0, 0)), c.runAction(cc.sequence(cc.spawn(cc.jumpBy(1, 0, 0, 300, 1), cc.scaleTo(1, 1)), cc.delayTime(1), cc.spawn(cc.moveTo(1, cc.v2(0, 500)), cc.scaleTo(1, 0)), cc.callFunc(function () {
          ...
          var e = r.default.Spawn("add1", cc.find("Canvas/mainGameUi"));
          e.position = cc.find("Canvas/mainGameUi").children[0].position.add(cc.v2(0, 50)), e.runAction(cc.spawn(cc.moveBy(.5, 0, 50), cc.fadeOut(.5)))
        }, t.prototype.playCustomSound = function (fruitNumber) {
          // 播放自定义音效，音频文件编号从2开始
          var audioFileNumber = fruitNumber + 1;
          var audioPath = window.DynamicAssetLoader.buildAssetPath("sounds/" + audioFileNumber + ".mp3");
          var audio = new Audio(audioPath);
          audio.volume = 0.5;
          audio.play().catch(function(error) {
            // 如果自定义音效播放失败，使用默认音效
            d.default.Instance.Play(2, !1, 1);
          });
